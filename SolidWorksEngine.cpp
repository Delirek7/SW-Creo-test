...
bool SolidWorksEngine::ConvertToObj(const std::string& outputPath) {
    ...
    try {
                                    ...
                                    if (psaStart && psaEnd) {
                                        ...
                                        if (vPoly.vt == (VT_ARRAY | VT_R8) && vPoly.parray != nullptr) {
                                            ...
                                            for (long k = 0; k < count; k += 3) {
                                                chain.push_back({ pDoubles[k], pDoubles[k+1], pDoubles[k+2] });
                                            }
        ...
        // Write Edges as continuous polylines
        ...
        std::cout << "Successfully exported " << allTriangles.size() << " triangles and " << allEdgeChains.size() << " edge chains to " << outputPath << std::endl;
        return true;
